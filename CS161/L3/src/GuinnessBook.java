...
public class GuinnessBook {
	...
	public GuinnessBook(String filename) throws FileNotFoundException{
		...
		while(scan.hasNextLine()){
			...
			Animal myAnimal = new Animal(animalName, topSpeed);
			landAnimals.add(myAnimal);
		}
		scan.close();
	}
	...
	public String toString(){
		...
		for(int i=0;i<landAnimals.size();i++) {
			output += "Name: " + landAnimals.get(i).getName() + " ";
			output += "Top Speed: " + landAnimals.get(i).getTopSpeed() +"\n";
		}
		return output;
	}
	...
	private void testGuinnessBook() throws FileNotFoundException {
        ...
        //Now print all the animals whose speed is around 50mph
         System.out.println("Animals speed around 50mph:");
         Animal fastAnimal = new Animal("rabbit", 50);
         for(int i=0;i<landAnimals.size();i++) {
        	 if(landAnimals.get(i).equals(fastAnimal)) {
