...
string Stem::Step1(string sIn) {
    ...
    // check to remove a 's suffix
        ...
        sIn = Replace(sIn, "", (sIn.size())-lSuffix); // remove everything after the apostrophe
...
string Stem::Step2(string &sIn) {
    //check the first row 
    if(Find(sIn,"sses"))
        sIn = Replace(sIn, "ss", (sIn.size())-lSuffix); //replace
...
string Stem::Step3(string &sIn) {
    if(Find(Region1(sIn),"eed") || Find(Region1(sIn),"eedly")) // row 1 including conditions
        sIn = Replace(sIn,"ee", (sIn.size())-lSuffix);
...
//i am so sorry for this mess.. just know... there lots of cases
...
const string Stem::step3Replace(string &sIn, const string &suffix) {
    ...
    sIn = Replace(sIn, "", (sIn.size())-suffix.size()); // replace the suffix with nothing
    if(preceder.size() < 2) {
        return sIn;
    }
...
string Stem::step7Region2(string &sIn) {
    ...
    string reg2 = Region2(sIn);  // region2 of the string
...
const bool Stem::Find(const string &sIn, const string &suffix) {
    ...
    // if the string is the entire suffix then retrun true
    if(sIn == suffix) {
        lSuffix = suLen;
        return true;
    }
