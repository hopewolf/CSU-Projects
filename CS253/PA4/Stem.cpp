...
string Stem::Step2(string &sIn) {
    ...
    // check the second row
        ...
        if(Preceder(sIn, "ied").length() > 1) // replacement conditions
            sIn = Replace(sIn, "i", sIn.find("ie"));
        else
            sIn = Replace(sIn, "ie", sIn.find("ie"));
    }
...
string Stem::Step5(string &sIn) {
    ...
    // 1.5 conditon
        ...
        sIn = Replace(sIn, "nce", (sIn.size())-lSuffix);
...
//TODO FIX SO THAT NONE OF THE THREE CHARS CAN BE WXY
