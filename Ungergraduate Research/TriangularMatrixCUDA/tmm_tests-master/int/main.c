 ...
 * 1) Using 2D matrices
 *
 * A, B and C are created as double-dimension arrays (int**)
 * where:
 * - A[i] is a pointer to a int* array (line i of matrix A)
 ...
 *
 * A, B and C are created as single-dimension arrays (int*)
 * where:
 * - A[i] is A(0,0)
 ...
 *
 * */
void TMM2D(long N, int** A, int** B, int** C)
{
...
void TMMlin(long N, int* A, int* B, int* C)
{
